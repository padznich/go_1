...
func IntSliceToString(sl []int) string {
	...
	sl_str := make([]string, len(sl))
	for i, v := range sl {
		sl_str[i] = strconv.Itoa(v)
	}
	out := strings.Join(sl_str, "")

	return out
}

func MergeSlices(sl_f []float32, sl_i []int32) []int {

	sl_fi := make([]int32, len(sl_f))
	for i, v := range sl_f {
		sl_fi[i] = int32(v)
	}
	tmp := append(sl_fi, sl_i...)
	out := []int{}
